...
awful.screen.connect_for_each_screen(function(s)
    ...
    local text_temperature = require("noodle.temperature")
    local text_temperature_icon = wibox.widget.textbox()
    text_temperature_icon.markup = helpers.colorize_text("", beautiful.xcolor2)
    text_temperature_icon.font = "Typicons 11"

    local temp = wibox.widget {
	    text_temperature_icon,
	    text_temperature,
	    spacing = dpi(8),
	    layout = wibox.layout.fixed.horizontal
    }

    local text_battery = require("noodle.battery")
    local text_battery_icon = wibox.widget.textbox()
    text_battery_icon.markup = helpers.colorize_text("", beautiful.xcolor3)
    text_battery_icon.font = "Typicons 11"
    ...
    -- Create the wibox
    s.mywibox = awful.wibar({ position = beautiful.wibar_position, screen = s, width = beautiful.wibar_width, height = beautiful.wibar_height, shape = helpers.rrect(beautiful.wibar_border_radius), bg = beautiful.bg_normal .. "99"})
